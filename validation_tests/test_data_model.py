    ...
    def test_data_modelr(self):
        ...
        for index, row in devices_df.iterrows():
            ...
            with open(os.path.join(path_input, "entity_templates", device_type + ".json")) as f:
                ...
                entity = ContextEntity(id=entity_template.get('id'), type=entity_template.get('type'))
                ...
                with cb_client:
                    ...
                    entity_fiware = cb_client.get_entity(entity_id=entity.id)
            ...
            with open(os.path.join(path_input, "device_templates", device_type + ".json")) as f:
                ...
                # 3. test the connection by sending some data
                with iotc:
                    iotc.post_device(device=device)
                    device_fiware = iotc.get_device(device_id=device.device_id)
                    self.assertEqual(device_fiware.device_id, device_template.get('device_id'))
